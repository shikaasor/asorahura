...
export interface FlameWrapOptions {
  ...
  scorch?: number;
  /**
   * LOCAL ADDITION. When false, one final frame is drawn and the render
   * loop parks itself. Lets several wrapped elements sit on a page without
   * each one burning a rAF loop and a shader pass while idle.
   */
  active?: boolean;
  /**
   * LOCAL ADDITION. When false, the html-in-canvas capture path is never
   * used even where the browser supports it, so the wrapped content stays
   * as real, interactive DOM. Required wherever the content contains links
   * or controls — a captured subtree renders as a texture.
   */
...
export function createFlameWrap(
  ...
  function frame(now: number) {
    ...
    render();
    // LOCAL ADDITION: park after drawing the frame that settles the effect
    // to rest, instead of idling a rAF loop and a shader pass per instance.
    if (!config.active) {
      running = false;
      return;
    }
...
export function FlameWrap({
  ...
  const native = supported && options.captureContent !== false && !failed;
